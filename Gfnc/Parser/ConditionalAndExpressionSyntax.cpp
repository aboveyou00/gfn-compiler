...
{
    ExpressionSyntax *ConditionalAndExpressionSyntax::tryParse(Cursor<Tokenizer::Token*> &cursor)
    {
    ...
    ConditionalAndExpressionSyntax *ConditionalAndExpressionSyntax::tryParseRhs(Cursor<Tokenizer::Token*> &cursor, ExpressionSyntax *lhs)
    {
    ...
    void ConditionalAndExpressionSyntax::emit(Emit::MethodBuilder &mb) const
    {
        auto shortCircuitTag = mb.createAnonymousTag();
        auto afterTag = mb.createAnonymousTag();

        this->lhs()->emit(mb);
        mb.addOpcode(new Emit::OpBrFalse(shortCircuitTag));
